...
!     ======================
      ...
      if (lrestart) then
          !>SiS for adm
          inquire(unit=nreaunit, opened=litsopen) 
          if ( litsopen ) then
           rewind(unit=nreaunit)
         else
           open(unit=nreaunit,file=yrfile,form='unformatted')
         end if
         nresnum = 0 
         !<SiS for adm
         do
...
!$TAF SUBROUTINE get_restart_integer  INPUT = 1
!$TAF SUBROUTINE get_restart_integer  OUTPUT = 2
...
!The following TAF flow directives are neccessary because the control flow of reseek
!  is irreducible (as of TAF-6.0.5) and the restart is called from the active scope (via tafreset):
!$TAF SUBROUTINE get_restart_array  INPUT = 1,3,4,5
!$TAF SUBROUTINE get_restart_array  OUTPUT = 2
!$TAF SUBROUTINE get_restart_array  ACTIVE = 2
!$TAF SUBROUTINE get_restart_array  ADKILL = 2
!
...
!$TAF SUBROUTINE get_restart_seed  INPUT = 1,3
!$TAF SUBROUTINE get_restart_seed  OUTPUT = 2
      ...
      do j = 1 , nresnum
         if (trim(yn) == trim(yresnam(j))) then
            call reseek(yn,j)
            read (nreaunit,iostat=iostat) ka(:)
            if (iostat.ne.0) then
              print*,"WARNING skipping random seed for portability" ! random seed length compiler dependent, removeme , sb
            end if
            nlastrec = nlastrec + 1
            return
         endif
