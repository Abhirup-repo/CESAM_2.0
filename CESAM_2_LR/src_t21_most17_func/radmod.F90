...
!
!
! SiS: copied from   subroutine solang
!      and           subroutine mko3
!      to the module to avoid compilatio errors 
!      both subroutines use radmod anyway.
...
!           Guokun modification 
!++++++++++++++++++++++++++++++++++++++++++++++++++++++
! guokun added to initialize parameter matirx 21.Nov
      th2ocg(1:NUGP)=th2oc
...
!$TAF STORE gdist2, gmu0 = inner1
!
!**   3) compute ozon distribution
!
      if(no3<3) call mko3
!$TAF STORE dqo3 = inner1
!
...
!$TAF init mko3n = static, nlev-1
      if (no3 == 1) then ! compute synthetic ozone distribution
         ...
         do jlev=NLEV,2,-1
!$TAF store zh, zo3t = mko3n, REC = jlev - 1
...
!$TAF init swr  = static, 1
!$TAF init swrn = static, NLEV
!
...
!$TAF store zb1,zrcl1s,zrcl2s,ztcl2s,zu,zexp = swrn
       where(losun(:) .and. (dcc(:,jlev) > 0.))
         zr(:)   = (zu(:)+1.)*(zu(:)+1.)*zexp(:)                      &
     &           - (zu(:)-1.)*(zu(:)-1.)/zexp(:)
...
!
      ...
      endwhere
      do jlev=1,NLEV
!$TAF STORE zxo3t, zywvt, zcs = swrn
       where(losun(:))
...
!
      do jlev=1,NLEV
!$TAF STORE zra1s,zra2s,zta1,zta1s,zta2,zta2s,zto3t,zto3tu,ztwvt,ztwvtu = swrn
       where(losun(:))
        zt1(:,jlev)=zta1(:)
        zt2(:,jlev)=zta2(:)
        zr1s(:,jlev)=zra1s(:)
...
!$TAF store zrb1(:,jlev),zrb1s(:,jlev),zto3 = swrn
      where(losun(:))
        zto3t(:)=zto3t(:)*zto3(:)
!
...
!$TAF store zrb2(:,jlev),zrb2s(:,jlev),ztwv = swrn
      where(losun(:))
       ztwvt(:)=ztwvt(:)*ztwv(:)
!
!     upward scattered beam
...
!
     ...
     &                       *z1mrabr(:)
       ...
       endwhere
      enddo
!$TAF STORE ztb1,ztb1u,ztb2,ztb2u,zrb1s, zrb2s = swr
      where(losun(:))
       zt1(:,NLEP)=zta1(:)
       zt2(:,NLEP)=zta2(:)
       zr1s(:,NLEP)=zra1s(:)
...
!
     ...
     &        +(1.-dls(:))*(1.-dicec(:))*AMIN1(0.05/(zmu0(:)+0.15),0.15)
       ...
       zra2(:)=dalb(:)
      endwhere
!$TAF STORE zr1s,zr2s,zra1,zra1s,zra2,zra2s,zt1,zt2 = swr
      do jlev=NLEV,1,-1
!$TAF STORE zra1,zra1s,zra2,zra2s = swrn
       where(losun(:))
        zrl1(:,jlev+1)=zra1(:)
        zrl2(:,jlev+1)=zra2(:)
        zrl1s(:,jlev+1)=zra1s(:)
...
!$TAF init lwrn  = static, NLEV
!$TAF init lwrnn = static, NLEV*NLEV
!
...
!
       do jlev2=jlev,NLEV
        ...
        zsumco2(:)=zzf2*zps2(:)
!$TAF store zsumwv,zsumo3,zsumco2,ztaucc = lwrnn
!
...
!
        ...
        endwhere
!$TAF store zao3, zth2o, zaco2, zah2o = lwrnn
!
...
!
       if(jlev == 1) then
        ...
        do jlev2=1,NLEV
         jlep2=jlev2+1
! FO: storing avoids trivial recomputation loop here. But consider code restructuring,
!     since the ratio ztaucs(:,jlev2)/ztau0(:) is 1 for all iterations jlev2>1.
!$TAF STORE ztau0 = lwrnn
...
!FO: Use (jlev-1) instead of jlem in taf store directive, because the size of the variable
!FO: to be stored must be accessible in a different scope when tapes are opened:
!$TAF STORE zbd(:,jlev-1:jlev),zbu = lwrn
!
!     fluxes
!
...
!
      ...
      enddo
!$TAF STORE dftd,zbd,ztausf = lwrn
