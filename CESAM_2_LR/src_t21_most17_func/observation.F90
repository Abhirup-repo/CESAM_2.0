...
!
      ...
      !   real,allocatable :: fobsav(:,:) ! time averaged field, partial
        ...
        real :: var_mean_atm(1:4)=(/0.436,0.343,0.414,0.264/)
        real :: weight_mean_atm(1:4)=(/.1,1.,1.,40./)
...
! open file for direct access SERVICE i/o
          uflux = get_free_unit(uflux)
#ifdef GENOBS
          call initsrvio(uflux,yname,'REPLACE')
#else
...
!          integer :: step ! number of cost function contribution
!
          use pumamod, only : NUGP,NLON,NLAT,NHOR,gaussw,lunstable,nstep0,nstep,nstep2,n_spinup,ncost,NROOT,mypid
...
!
          if(mypid==NROOT) then
          open(410,file='cost_contribution.txt',form='formatted')
          endif
          if(mypid==NROOT) then
           write (410,'(1x,a,i7,a,i7)') 'cost function contribution #',step,' step #',nstep-nstep0+1
          endif
          do iobs=1,nobs
             irec  = nobs*(step-1)+iobs
             if (mypid==NROOT) then
...
!$TAF STORE fm,frd = obsn      
             do jlat=1,NLPP
                  gmm = gmm + gaussw(jlat) * sum(fm((jlat-1)*NLON+1:jlat*NLON))
                  gmo = gmo + gaussw(jlat) * sum(frd((jlat-1)*NLON+1:jlat*NLON))
              ENDDO
             call mpsumr(gmm,1)
             call mpsumr(gmo,1)
             if(mypid==NROOT) then
                 gmm = gmm  / sum(gaussw(1:NLAT)) / NLON
                 gmo = gmo  / sum(gaussw(1:NLAT)) / NLON
              endif
             call mpbcr(gmm)
             call mpbcr(gmo)
!               write (*,'(2e20.12)') gmm,gmo

            endif

             ch = 0.
             nval = 0.
!$TAF STORE gmm, gmo= obsn             
             do jlat=1,NLPP
               ...
               if(obscode(iobs)==278 .or. obscode(iobs)==176 .or. obscode(iobs)==177 .or. &
                     obscode(iobs)==178 .or. obscode(iobs)==179 )then
                 where (lval)
                    fh(:) = fm((jlat-1)*NLON+1:jlat*NLON) - frd((jlat-1)*NLON+1:jlat*NLON) -gmm + gmo        
                 end where
                else
...
!$TAF STORE fh = obsnh
                   if(obscode(iobs)==278)then
                     fh(:) = fh(:) * sqrt(fobserr((jlat-1)*NLON+1:jlat*NLON,iobs))
                     ch(1) = ch(1) + gaussw(jlat) * sum(fh(:))
                   else
             ...
             if(mypid==NROOT) then
               ...
               if(obscode(iobs)==278 )then
               !  if(nstep-nstep0+1 .lt. 129600)then ! set to 10 year
               !   ch(1) = ch(1) * NLAT / sum(gaussw(1:NLAT)) / nval
               !   costm = costm + ch(1) 
               ! write (410,'(1x,a,i3,a,i4,a,i6,a,e20.12,a,i5)') 'cost function contribution of obs ',iobs, &
               !    ' code (Radiation) ',obscode(iobs),' level ',obslev(iobs),' = ', &
               !    ch(1),' missing:',NUGP-nval
               !  elseif(nstep-nstep0+1 .eq. 129600)then
               !    costm = costm + ch(1) 
               !    cost = cost + (costm)**2 * 0.5 * fobserrfac(iobs) 
               !  write (410,'(1x,a,i3,a,i4,a,i6,a,e20.12,a,i5)') 'cost function contribution of obs ',iobs, &
               !    ' code ',obscode(iobs),' level ',obslev(iobs),' = ', &
               !    (costm)**2 * 0.5 * fobserrfac(iobs),' missing:',NUGP-nval
               ! endif

             elseif(obscode(iobs)==176 )then
                write (410,'(1x,a,i3,a,i4,a,i6,a,2e20.12,a,i5)') 'cost function contribution of obs ',iobs, &
                  ' code ',obscode(iobs),' level ',obslev(iobs),' = ', &
                  ch(1) * 0.5 * fobserrfac(iobs) * NLAT / sum(gaussw(1:NLAT)) / nval,  &
                  weight_mean_atm(1)*(gmm-gmo)**2/var_mean_atm(1)**2,' missing:',NUGP-nval
               cost = cost + weight_mean_atm(1)*(gmm-gmo)**2/var_mean_atm(1)**2+ &
                 ch(1) * 0.5 * fobserrfac(iobs) * NLAT / sum(gaussw(1:NLAT)) / nval ! mean square deviation per

             elseif(obscode(iobs)==177 )then
                write (410,'(1x,a,i3,a,i4,a,i6,a,2e20.12,a,i5)') 'cost function contribution of obs ',iobs, &
                  ' code ',obscode(iobs),' level ',obslev(iobs),' = ', &
                  ch(1) * 0.5 * fobserrfac(iobs) * NLAT / sum(gaussw(1:NLAT)) / nval,  &
                  weight_mean_atm(2)*(gmm-gmo)**2/var_mean_atm(2)**2,' missing:',NUGP-nval
               cost = cost + weight_mean_atm(2)*(gmm-gmo)**2/var_mean_atm(2)**2+ &
                 ch(1) * 0.5 * fobserrfac(iobs) * NLAT / sum(gaussw(1:NLAT)) / nval ! mean square deviation per

             elseif(obscode(iobs)==178 )then
               write (410,'(1x,a,i3,a,i4,a,i6,a,2e20.12,a,i5)') 'cost function contribution of obs ',iobs, &
                  ' code ',obscode(iobs),' level ',obslev(iobs),' = ', &
                  ch(1) * 0.5 * fobserrfac(iobs) * NLAT / sum(gaussw(1:NLAT)) / nval,  &
                  weight_mean_atm(3)*(gmm-gmo)**2/var_mean_atm(3)**2,' missing:',NUGP-nval
               cost = cost + weight_mean_atm(3)*(gmm-gmo)**2/var_mean_atm(3)**2+ &
                 ch(1) * 0.5 * fobserrfac(iobs) * NLAT / sum(gaussw(1:NLAT)) / nval ! mean square deviation per

             elseif(obscode(iobs)==179 )then
               write (410,'(1x,a,i3,a,i4,a,i6,a,2e20.12,a,i5)') 'cost function contribution of obs ',iobs, &
                  ' code ',obscode(iobs),' level ',obslev(iobs),' = ', &
                  ch(1) * 0.5 * fobserrfac(iobs) * NLAT / sum(gaussw(1:NLAT)) / nval,  &
                  weight_mean_atm(4)*(gmm-gmo)**2/var_mean_atm(4)**2,' missing:',NUGP-nval
               cost = cost + weight_mean_atm(4)*(gmm-gmo)**2/var_mean_atm(4)**2+ &
                 ch(1) * 0.5 * fobserrfac(iobs) * NLAT / sum(gaussw(1:NLAT)) / nval ! mean square deviation per

             else
                write (410,'(1x,a,i3,a,i4,a,i6,a,e20.12,a,i5)') 'cost function contribution of obs ',iobs, &
                  ' code ',obscode(iobs),' level ',obslev(iobs),' = ', &
                  ch(1) * 0.5 * fobserrfac(iobs) * NLAT / sum(gaussw(1:NLAT)) / nval,' missing:',NUGP-nval
               cost = cost + ch(1) * 0.5 * fobserrfac(iobs) * NLAT / sum(gaussw(1:NLAT)) / nval ! mean square deviation per grid cell
...
! ----------------------------------------------------------------------------------------------
   subroutine nudgini
    ...
    real(kind=ioprec):: fio1(NLON*NLAT), fio2(NLON*NLAT) ! single data field in single precision
       ...
       open (junit,file=nfile, FORM='UNFORMATTED',ACCESS='DIRECT',RECL=recl,STATUS='old',action='read', convert='LITTLE_ENDIAN')
...
! observations start from 06:00-30-12-2000 to 00-00-02-01-2002
 ! just for coding convinence 
 ! open file
  !print*,"------------------------------------------------"
  !print*,"----------------calling getnudge--------------------------------"
