...
!!    for MIT coupling 
      real :: stotf         = 0.054! factor to convert sal.(psu) in tfreeze (C)
      real :: dcool         = 50.00! depth of ocean ML intercting with ice
!!

!
!     global integer
!
      integer :: nud        = 106    ! unit for messages
...
!!    for MIT coupling
      real :: xsss(NHOR)      = 35.  ! sea surface salinity (psu/ppt)
      real :: xtfreeze(NHOR)  = 0.   ! freezing T dependent on sss (K)
      real :: xps(NHOR)       = 0.   ! atm. surface pressure (Pa)
!!
...
!!    for MIT coupling
      integer :: nmit  = 0          ! mit coupling switch (namelist)
      real    :: xsaltflx(NHOR) = 0.! salt flux into ocean
      real    :: osst(NHOR) = 0.    ! mit-sst
      real    :: osss(NHOR) = 0.    ! mit-sss
!!
...
!
      if(nmit > 0) then
       ...
       if(nrestart /= 0) then
        if(nmit < 99) then      !!FLTEST: enable one way coupling plasim -> MIT for test
         where(xls(:) < 1.)
          xsst(:)=osst(:)
          xsss(:)=osss(:)
          xoflux(:)=0.
!      for tfreeze=f(SSS) (but actual version: tfreeze= -1.9 degC)
!         xtfreeze(:)=-1.*xsss(:)*stotf+TMELT
         endwhere
        endif
       endif
      endif
...
!
      real :: zsnowold(NHOR),zcflux(NHOR),zcflux2(NHOR)
      real :: zicedold(NHOR)

!!    for MIT coupling
      real :: zheat(NLON,NLAT) = 0.   ! net total heat flux to mit-ocean
      real :: zswr(NLON,NLAT)  = 0.   ! net short wave radiation to mit
      real :: zfresh(NLON,NLAT)= 0.   ! net fresh water flux (w/o roff) to mit
      real :: zroff(NLON,NLAT) = 0.   ! runoff to mit
      real :: ztaux(NLON,NLAT) = 0.   ! u-stress to mit
      real :: ztauy(NLON,NLAT) = 0.   ! v-stress to mit
      real :: zps(NLON,NLAT)   = 0.   ! surface pressure to mit
      real :: zicem(NLON,NLAT) = 0.   ! snow+ice mass to mit
      real :: zicemp(NHOR)     = 0.   ! snow+ice mass (partial field)
      real :: ztauxp(NHOR)     = 0.   ! taux modified (partial field)
      real :: ztauyp(NHOR)     = 0.   ! tauy modified (partial field)
      real :: zfreshp(NHOR)    = 0.   ! fresh w.f. modified (partial field)  
      real :: zswrp(NHOR)      = 0.   ! short wave rad. modified (partial field)
!!
...
!$TAF store xcflux,xscflx = icestep
!
!     make new ice
!
!     a) thickness
!
      call mkice
!$TAF store xcflux, xiced = icestep
!
...
!
      if (nprint==2) then
       ...
       zcflux2(:)=xaheat(:)+xoheat(:)-xtsflux(:)-xsmflx(:)-ximelt(:)     &
     &          -xcflux(:)+xcfluxr(:)
       call mpgagp(zprf1,zcflux2,1)  
...
!!     potential modifications for other oceans
!
...
!$TAF store xcflux,xicec,cheat = icestep
...
!
     ...
     &            -(xiced(:)-zicedold(:))/xdt*CRHOI/1.E3
       endwhere 
!
!     b) put atm.-variables to coupler
!
       call mpgagp(zheat,xcflux,1)
       call mpgagp(zswr,zswrp,1)
       call mpgagp(zfresh,zfreshp,1)
       call mpgagp(zroff,xroff,1)
! no ps effect     call mpgagp(zps,xps,1)
! no ice effect    call mpgagp(zicem,zicemp,1)
...
! no ps effect     call cputpsa(zps)
! no ice effect    call cputima(zicem)
        call cputtaua(ztaux,ztauy)
       endif
!
!     c) overwrite oceanic sst with mit-values
!        (the oceanic variables are read in *getmitocean*)
!
!$TAF INCOMPLETE xsst,xoflux
       if(nmit < 99) then      !!FLTEST: enable one way couplning plasim -> MIT for test
        where(xls(:) < 1.)
         xsst(:)=osst(:)
         xsss(:)=osss(:)
         xoflux(:)=0.
! no sss dependent tfreeze   xtfreeze(:)=-1.*xsss(:)*stotf+TMELT
         xtfreeze(:)=TFREEZE
        endwhere
       endif 
      endif
...
!!    for MIT coupling
!old      where(xsst(:) <= TFREEZE .or. xiced(:) > 0.)               
...
!!    for MIT coupling
!
!     melt sea ice if sst > tfreeze and take heat from ocean
!
      if(nmit > 0) then
       zmelt(:)=0.
...
!!    for MIT coupling
        write(nud,*) 'tfreeze: ',zprf5(nprhor)
        write(nud,*) 'flux from ocean because t > tf: ',zprf4(nprhor)
!!
