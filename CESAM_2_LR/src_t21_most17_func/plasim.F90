...
!     plasimversion = "Most17 for MIT coupling: December 2014 by Larry"
...
!!    for MIT coupling
      ...
      plasimversion = "Most17 for MIT coupling: April 2014 by Larry"
!
!     moved from main into prolog
!
      call setfilenames
      call opendiag
      if (mrnum == 2) then
...
!!    for MIT coupling
      call mpscrn(gaussw ,NLPP) ! gaussian weights 
!!
...
!
      call mpbci(makenudge)
      call mpbci(ncost)    ! fastopt cost function parameter
      call mpbci(n_spinup) ! fastopt cost function parameter

      call mpbcrn(dnudg, NLEV)
      call mpbcrn(znudg, NLEV)
      call mpbcrn(qnudg, NLEV)
      call mpbcrn(tnudg, NLEV)
      call mpbcr(pnudg)
...
!!    for MIT coupling
      if (nrestart == 0) nstep = n_start_step ! timestep since 01-01-0001
      call updatim(nstep)  ! set date & time array ndatim
      if(mypid == NROOT) write(nud,'(a,i4.4,"-",i2.2,"-",i2.2," ",i2.2    &
     &                        ,":",i2.2," UTC")')                       &
     &                   'Simulations starting ',ndatim(1),ndatim(2)    &
     &                   ,ndatim(3),ndatim(4),ndatim(5)
      nstep0 = nstep ! FastOpt, remember start timestep

      call cal2step(nstepe,ntspd,ndatim(1)+(ndatim(2)+n_run_months)/12  &
     &             ,mod(ndatim(2)+n_run_months,12),ndatim(3)+n_run_days &
     &             ,ndatim(4),ndatim(5)) 

      nstepe = nstepe + n_run_steps + n_spinup ! added, FastOpt

      call updatim(nstepe)  ! set date & time array ndatim
      if(mypid == NROOT) write(nud,'(a,i4.4,"-",i2.2,"-",i2.2," ",i2.2    &
     &                        ,":",i2.2," UTC")')                       &
     ...
     &                   ,ndatim(3),ndatim(4),ndatim(5)
      ...
      dprl(:)=0. ! added, FastOpt
      dprc(:)=0. ! added, FastOpt
      dprs(:)=0. ! added, FastOpt
      dftu(:,:)  = 0.0 ! added, FastOpt
      if (ndiag==0) then ! FastOpt
         ndiag = nstepe
         print'("ndiag set to ",i8)',ndiag ! FastOpt
         call mpbci(ndiag   ) ! write diagnostics interval
      end if ! FastOpt
!!
...
!!    for MIT coupling
!
!     new subroutine to do the short initialization time steps
!
...
!     ======================
      ...
      subroutine init_master
      use pumamod
      use icemod, only : cheat, xmld, xsndch, xflxice2,xheat,xicec,xiced,xoflux,xprs,xqmelt,xsnow,xsst,xts ! FastOpt
      use seamod, only : clhdta,clhfla,clwfla,cpmea,croffa,cshdta,cshfla,cswfla,ctauxa,ctauya,cust3a,cheata, &
           &           cprsa, cicec, ciced, cmld, csmelt, csndch, csnow, csst, cts, dqs  ! FastOpt
      use seamod, only : naccua ! FastOpt
      use seamod, only : dtt=>dts
      use landmod, only : nlsoil, driver, dsnowz, dsoilt, dtclim, dtsm, dwclim ! FastOpt
      use landmod, only : dtu=>dts,dqs_land=>dqs
      use vegmod, only : valb_max,vz0_min,zvalb,dlai,zvrhs,dvsoil,zveg,zvz0,ibiomass ! FastOpt
      use radmod, only : gmu0,dqo3,gdist2 ! FastOpt
      use rainmod, only : icclev, icctot ! FastOpt
      use oceanmod, only : ysst,nlev_oce ! FastOpt
      use oceanmod, only : nstep_ocean=>nstep ! FastOpt
      use icemod, only : naccuo ! FastOpt
      use observation, only : genav,addcost,writeobs,nobs,cost
      implicit none ! FastOpt
      integer :: ikits, jkits, nscd, iyea, imon
!     ***************************
...
!!    for MIT coupling
!      subroutine master
     subroutine master(ksteps,kstepo)
!!
     use mpimod, only : MPI_COMM_WORLD ! required for TAF parallel tapes
     use pumamod
     use icemod, only: naccuo,xpme,xswfl
     use icemod, only: xtaux,xtauy
     use icemod, only : cheat,xmld,xsndch,xflxice2,xheat,xicec,xiced,xoflux,xprs,xqmelt,xsnow,xsst,xts 
     use icemod, only : nstep_ice => nstep
     use icemod, only : osst
     use seamod, only : cheata,clhdta,clhfla,clwfla,cpmea,croffa,cshdta,cshfla,cswfla,ctauxa,ctauya,cust3a,cheata,&
          &           cprsa,cicec,ciced,cmld,csmelt,csndch,csnow,csst,cts  
     use seamod, only : naccua 
     use seamod, only : dts_sea=>dts
     use seamod, only : dqs_sea=>dqs
     use landmod, only: devap,dalb,dforest
     use landmod, only : nlsoil,driver,dsnowz,dsoilt,dtclim,dtsm,dwclim,dwater 
     use landmod, only : dts_land=>dts,dqs_land=>dqs
     use vegmod, only : valb_max,vz0_min,zvalb,dlai,zvrhs,dvsoil,zveg,zvz0,ibiomass, & 
          &             dcsoil,dcveg,znogrow,zres
     use radmod, only : gmu0,dqo3,gdist2 
     use rainmod, only : icclev,icctot,dprcl,dprll,dprscl,dprsll 
     use oceanmod, only : ysst,nlev_oce,yifluxr 
     use oceanmod, only : nstep_ocean=>nstep 
     use icemod, only : naccuo,xtfreeze,xroff,xicecc 
     use observation, only : genav,addcost,writeobs,nobs,cost 
     use cplmod, only : nxa,nya,ahfla,apmea,aroffa,aswra,atauxa,atauya
     use cplmod, only : nhfla,npmea,nroffa,nswra,ntaua
     use calmod, only : monaccu
     implicit none 
     integer :: koutdiag
     integer :: ikits, jkits, iyea, imon !, nscd
     integer :: ksteps,jstep   ! LARRY
     integer :: kstepo ! step of ocean loop, FastOpt
!--   Initialise storage for reference trajectory with check-
!--   pointing.
#ifdef STATIC_MEMTAPE
! ksteps is currently the number of atm steps per ocean step
!$TAF INIT calcgpn = STATIC, (NLEV-2)*ksteps
!$TAF INIT inner1 = STATIC, ksteps
!$TAF INIT innern = STATIC, NLEV*ksteps
!$TAF INIT inner2n = STATIC, (2*NLEV+1)*ksteps
!$TAF INIT innernn2 = STATIC, ((NLEV+1)*NLEV/2)*ksteps
!$TAF INIT inner2nn = STATIC, (2*NLEV+1)*(NLEM-1)*ksteps
!$TAF INIT inners = STATIC, NLSOIL*ksteps
!$TAF INIT innerno = STATIC, (NLEV_OCE-2)*ksteps
!$TAF INIT obs1 = STATIC, (1+10*nchklev_1*nchklev_2/ncost)
!$TAF INIT obsn = STATIC, nobs*(1+10*nchklev_1*nchklev_2/ncost)
!$TAF INIT obsnh = STATIC, nobs*(1+10*nchklev_1*nchklev_2/ncost)*NLAT
#else
!$TAF INIT calcgpn  = 'tapes/calcgpn'
!$TAF INIT inner1   = 'tapes/inner1'
!$TAF INIT innern   = 'tapes/innern'
!$TAF INIT inner2n  = 'tapes/inner2n'
!$TAF INIT innernn2 = 'tapes/innernn2'
!$TAF INIT inner2nn = 'tapes/inner2nn'
!$TAF INIT innerh   = 'tapes/innerh'
!$TAF INIT inners   = 'tapes/inners'
!$TAF INIT innerno  = 'tapes/innerno'
!$TAF INIT obs1     = 'tapes/obs1' 
!$TAF INIT obsn     = 'tapes/obsn' 
!$TAF INIT obsnh    = 'tapes/obsnh'
#endif

     
!!    for MIT coupling
!
!     short initialization time steps moved to master_ini
! 
!     ***************************
!     * short initial timesteps *
!     ***************************
...
!     do while (mocd > 0 .or. nscd > 0)  ! main loop
      ...
      do nstep_inner = 0, ksteps -1 ! FastOpt - month/day countdown included
!        nstep2 = (ikey_dynamics-1)*ksteps + nstep_inner ! tape counter
        nstep  = nstep0 + (kstepo-1)*ksteps + nstep_inner ! time counter
        !print*,'current step is',nstep
!$TAF store nstep, naccua, naccuo, ndatim = inner1
!!
!$taf STORE &
!$taf& dcsoil,dforest,dicec,dql,dqt,drhs,dsnow,dsnowz,dsoilt,dtclim,dtsm,dvsoil,  &
!$taf& dwatc,dwclim,dwmax,xicec,xiced,ysst,zveg &
!$taf& = inner1

!$taf STORE &
!$taf&      cheata,cprsa,dalb,dcc,dflux,dlwfl,dprcl,dprll,dprscl,dprsll,dq,dqo3,dsnowz,dsoilt,dswfl,dt,&
!$taf&      dts_land,dts_sea,dtclim,dtsa,dtsm, &
!$taf&      du,dv,dz0,gdist2,gmu0,monaccu,naccua,ndatim,nstep,sd,sdm,sdp,sp,spm,spp,sq,sqm,sqp,st,stm,stp,sz,szm,szp,&
!$taf&      osst,xicec,xiced,xsnow,xsst,xts  = inner1
!$TAF store &
!$TAF& ahfla,apmea,aroffa,aswra,atauxa,atauya,cheat,cpmea,croffa,cswfla,ctauxa,ctauya,driver,&
!$TAF& drunoff,dwater,ndivdamp,nhfla,npmea,nroffa,nstep,nswra,ntaua,xheat,xicecc,xmld,xoflux,xpme, &
!$TAF& xprs,xqmelt,xroff,xswfl,xtaux,xtauy,yifluxr = inner1
!$TAF STORE dcveg = inner1, IF (nveg>0)
!        ************************************************************
!        * calculation of non-linear quantities in grid point space *
!        ************************************************************

         call gridpointa
!$TAF STORE sqt, gq = inner1
!        ******************************
!        * adiabatic part of timestep *
!        ******************************

         call spectrala
!$TAF STORE sdp,sqp,stp,szp,naccuo,nstep_ocean,xtfreeze = inner1
!$TAF STORE sd,sp,sq,st,sz,dq,dt,du,dv,dw = inner1
!        *****************************
!        * diabatic part of timestep *
!        *****************************

         call gridpointd
!$TAF STORE dp,dq,dqt,dt,sdt,sqt,stt,szt = inner1
!$TAF STORE ndatim = inner1
!$TAF store &
!$TAF& ahfla,apmea,aroffa,aswra,atauxa,atauya,cheat,cpmea,croffa,cswfla,ctauxa,ctauya,dcveg,driver,&
!$TAF& drunoff,dwater,ndivdamp,nhfla,npmea,nroffa,nstep_ice,nstep_ocean,nswra,ntaua,xheat,xicecc,xmld,xoflux,xpme, &
!$TAF& xprs,xqmelt,xroff,xswfl,xtaux,xtauy,yifluxr = inner1
...
!$TAF STORE sdm,spm,spp,spt,sqm,stm,stt,szm = inner1
         call spectrald
!
        if (ncost.ne.0) call genav ! FastOpt: preparing averages for cost function
!         
         call outaccu
...
!$TAF STORE nstep = inner1
! ???ION???
          if (ncost.ne.0) then
           if (mod(nstep-n_spinup-nstep0+1,ncost) == 0 .and. (nstep-nstep0) .gt. n_spinup ) then
#ifndef GENOBS
              call addcost((nstep-n_spinup-nstep0+1)/ncost) ! FastOpt: adding contribution to cost function
#else /* write obs */
              call writeobs((nstep-n_spinup-nstep0+1)/ncost) ! FastOpt: saving pseudo-observations
#endif
! ???ION???
           endif
          endif
          ...
          if (mod(nstep,nafter) == 0) then
             call outreset ! moved here, FastOpt
          endif
...
!         if (nshutdown   > 0) exit ! 'exit' instead of 'return', to make routine TAF-reducible
!         if (nscd  > 0) then
!            nscd = nscd - 1
!            if (nscd == 0) return
!         endif
...
!!    for MIT coupling
      call mpputgp('dtaux'  ,dtaux  ,NHOR,1)
      call mpputgp('dtauy'  ,dtauy  ,NHOR,1)
      call mpputgp('dp'     ,dp     ,NHOR,1)
!!
...
!!    for MIT coupling: no resource stats
!
!
!     if (mypid == NROOT) then 
...
!     read scalars and full spectral arrays
      ...
      if (mypid == NROOT) then
         !>SiS 2023.08 activate next 2 lines :
         call random_seed(size=nseedlen)
!SiS! if condition for adm         
         if(allocated (meed) ) then
           deallocate (meed)
         endif
         allocate(meed(nseedlen))
         ...
         !>SiS 2023.08 activate next line :
...
!!    for MIT coupling
      call mpgetgp('dtaux'  ,dtaux  ,NHOR,   1)
      call mpgetgp('dtauy'  ,dtauy  ,NHOR,   1)
      call mpgetgp('dp'    ,dp    ,NHOR,   1)
!!
...
!     =================
      ...
      namelist /plasim_nl/ &
                   ...
                   , ncost,n_spinup,tnudg, dnudg,znudg,qnudg,pnudg,chyear,makenudge ! added, FastOpt
!
...
!     compute internal diffusion parameter (LAUERSON)
      ...
      if (makenudge==1) then
            ...
            do jlev=1,NLEV
            if (tnudg(jlev)  > 0.0) then
            tnudg(jlev)  = 1./(TWOPI*tnudg(jlev))
            else
            tnudg(jlev)  = 0.
            endif
            if (dnudg(jlev)  > 0.0) then
            dnudg(jlev)  = 1./(TWOPI*dnudg(jlev) )
            else
            dnudg(jlev)  = 0.
            endif
            if (znudg(jlev)  > 0.0) then
            znudg(jlev)  = 1./(TWOPI*znudg(jlev) )
            else
            znudg(jlev)  = 0.
            endif
            if (qnudg(jlev)  > 0.0) then
            qnudg(jlev)  = 1./(TWOPI*qnudg(jlev) )
            else
            qnudg(jlev)  = 0.
            endif
            enddo
...
!     ==================
      ...
      if(mpstep /= 1440/ntspd) then
	    write(*,*)'WARNING: mismatch in time variables:'
        write(*,*)'mpstep (minutes per step)=',mpstep,' ntspd (number of time steps per day)=',ntspd
	  end if
...
!     * inner levels *
!     ****************
!$TAF incompl gtn,gqn,guz,gvz,dw
      do jlev = 2 , NLEV-1
...
!     =====================
      ...
      use seamod, only: dqs_sea => dqs
      use seamod, only: dts_sea => dts
      use radmod, only: dqo3, gmu0,gdist2
      use calmod, only : monaccu
      use seamod, only : naccua
#ifdef STATIC_MEMTAPE
      use landmod, only : NLSOIL
      use oceanmod, only : NLEV_OCE
#endif
!      implicit none   ! added, FastOpt
      integer :: jlev,jlat ! added, FastOpt
!
      real sdf(NESP,NLEV)
      real stf(NESP,NLEV)
      real sqf(NESP,NLEV)
...
!$TAF INCOMPL dtdt,dq
      call miscstep
!$TAF store gqdt,dq,dtdt,dtsa,dz0 = inner1
...
!$TAF store dp,drhs,dt,du,dv,dq = inner1     
      call fluxstep
!$TAF store devap,dlhfl,dshfl,dtaux,dtauy,dtsa,dudt,dvdt,dtdt,dqdt,dlhdt,dshdt,dust3 = inner1     
!$TAF store dp,dt,dq,drhs = inner1     
!
!     dbug print out
!
...
!$TAF STORE gdist2,dalb,dcc,dqo3,gmu0,dfd,dflux,dlwfl,dswfl,dtdt = inner1     
!
!     dbug print out
!
...
!
!$TAF STORE dp,dprc,dprl,dprs,dq,dqdt = inner1
      call surfstep
...
!     ====================
      ...
      subroutine spectrald
      use pumamod
      use observation, only : getnudg, getnudg2d
...
!!! Nuddging field
      ...
      real :: frd(NUGP,NLEV)   = 0.0     ! temperature 
      real :: frdt(NHOR,NLEV)   = 0.0     ! temperature
      real :: srdt(NESP,NLEV)   = 0.0     ! temperature
      real :: srdtp(NSPP,NLEV)  = 0.0  ! Spectral Temperature Partial
...
! nudging on divergence
     ...
     srdtp=0.
     if(mypid==NROOT) then
      call getnudg(nstep,155,frd) ! nudging for former step:divergence
     endif
      call mpscgp(frd,frdt,NLEV)
      frdt=frdt/ww  ! nondimesionalize
      do jlev=1,NLEV
        call gp2fc(frdt(:,jlev),NLON,NLPP)  
        call fc2sp(frdt(:,jlev),srdt(:,jlev))
      enddo
       call mpsumsc(srdt,srdtp,NLEV)

     do jlev=1,NLEV 
! implicit calculated
      !print*, "Nudding is taking place"
       sdp(:,jlev)= sdp(:,jlev)+ delt2*dnudg(jlev) *(srdtp(:,jlev)-sdp(:,jlev))/(1.+dnudg(jlev) *delt2) 
     enddo 

!nudging on vorticity
     frd=0. ! reset frdt
     frdt=0. ! reset frdt
     srdt=0.
     srdtp=0.
     if(mypid==NROOT) then
call getnudg(nstep,138,frd) ! nudging for former step:vorticity
     endif
     call mpscgp(frd,frdt,NLEV)
     frdt=frdt/ww    ! nondimensionalize      
     do jlev=1,NLEV
       call gp2fc(frdt(:,jlev),NLON,NLPP)  
       call fc2sp(frdt(:,jlev),srdt(:,jlev))
 !      srdt(3,jlev)=srdt(3,jlev)+EZ    
     enddo     
    call mpsumsc(srdt,srdtp,NLEV)
    if (mypid == NROOT) then
         srdtp(3,:) = srdtp(3,:) +EZ ! transform to relative vorticity.
    endif

    
     do jlev=1,NLEV
!implicit nudging
       szp(:,jlev)= szp(:,jlev)+delt2*znudg(jlev) *(srdtp(:,jlev)-szp(:,jlev))/(1.+znudg(jlev) *delt2)
     enddo

!! nudging on temperature
      frd=0. ! reset frdt
      frdt=0. ! reset frdt
      srdt=0.
      srdtp=0.
     if(mypid==NROOT) then
      call getnudg(nstep,130,frd) ! nudging for former step:temperature
     endif      
     call mpscgp(frd,frdt,NLEV)
      do jlev=1,NLEV
        frdt(:,jlev)=frdt(:,jlev)/ct-t0(jlev)  ! nondimesionalize
      end do
      do jlev=1,NLEV
        call gp2fc(frdt(:,jlev),NLON,NLPP)  
        call fc2sp(frdt(:,jlev),srdt(:,jlev))
      ...
      do jlev=1,NLEV
!implicit nudging
        stp(:,jlev)= stp(:,jlev)+delt2*tnudg(jlev) *(srdtp(:,jlev)-stp(:,jlev))/(1.+tnudg(jlev) *delt2)
      enddo
...
!nudging on moist 
      ...
      srdtp=0.
     if(mypid==NROOT) then
      call getnudg(nstep,133,frd) ! nudging for former step:moist
      endif
        call mpscgp(frd,frdt,NLEV)

     do jlev=1,NLEV
       frdt(:,jlev)=frdt(:,jlev)*dp(:)/psurf  ! nondimesionalize
      ENDDO
      do jlev=1,NLEV
         call gp2fc(frdt(:,jlev),NLON,NLPP)  
         call fc2sp(frdt(:,jlev),srdt(:,jlev))
      ...
      do jlev=1,NLEV   
!implicit nudging
       sqp(:,jlev)= sqp(:,jlev)+delt2*qnudg(jlev) *(srdtp(:,jlev)-sqp(:,jlev))/(1.+qnudg(jlev) *delt2)
      enddo
...
!nudging on pressure
      ...
      srdtp=0.
      if(mypid==NROOT) then
      call getnudg2d(nstep,134,frd(:,1)) ! nudging for former step:pressure
      endif
      call mpscgp(frd(:,1),frdt(:,1),1)
      frdt(:,1)=log(frdt(:,1)/psurf)
      call gp2fc(frdt(:,1) ,NLON,NLPP)  ! fft
      call fc2sp(frdt(:,1),srdt(:,1))
      
      call mpsumsc(srdt,srdtp,NLEV)
      spp(:)=spp(:)+delt2*pnudg *(srdtp(:,1)-spp(:))/(1.+pnudg *delt2)
      if (mypid == NROOT) then
         spp(1) = 0.0
         spp(2) = 0.0
      endif
