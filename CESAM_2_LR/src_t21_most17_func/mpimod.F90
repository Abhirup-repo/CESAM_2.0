...
!     =================
      ...
      integer k(*),kh(n*npro)
! MPI implementation may not tolerate an overlap of send and receive buffer, therefore kh is introduced.      
      if (mypid==NROOT) kh = k(1:n*npro)
      call mpi_scatter(kh,n,mpi_itype,k,n,mpi_itype                  &
...
!     =================
      ...
      real p(*),ph(n*npro)
! MPI implementation may not tolerate an overlap of send and receive buffer, therefore ph is introduced.
      if (mypid==NROOT) ph = p(1:n*npro)
      call mpi_scatter(ph,n,mpi_rtype,p,n,mpi_rtype,NROOT,myworld,mpinfo)
...
!     =================
      ...
      real (kind=8) :: p(*),ph(n*npro)
! MPI implementation may not tolerate an overlap of send and receive buffer, therefore ph is introduced.
      if (mypid==NROOT) ph = p(1:n*npro)
      call mpi_scatter(ph,n,MPI_REAL8,p,n,MPI_REAL8,NROOT,myworld,mpinfo)
...
!     =================
      ...
      use mpimod
      implicit none
      real pcs(NLAT,NLEV), pcsh(NLPP)
      integer :: jlev
      do jlev = 1 , NLEV
! MPI implementation may not tolerate an overlap of send and receive buffer, therefore pcsh is introduced.
        pcsh = pcs(1:NLPP,jlev)
        call mpi_gather(pcsh(1),NLPP,mpi_rtype                           &
...
!     ==================
      ...
      allocate(ympname(npro)) ; ympnamemy = ' '
      call mpi_get_processor_name(ympnamemy,ilen,mpinfo)
! snb: send and receive buffers must not alias
      call mpi_gather(ympnamemy,80,MPI_CHARACTER,   &   
...
!!    new subs for MIT coupling & fastopt:

!     =================
!     SUBROUTINE MPBCLN
!     =================

      subroutine mpbcln(k,n) ! broadcast n logical
      use mpimod
      logical k(n)

      call mpi_bcast(k,n,mpi_ltype,NROOT,myworld,mpinfo)

      return
      end subroutine mpbcln

!     ====================
!     SUBROUTINE MPSUMI
!     ====================

      subroutine mpsumi(ki,kdim) ! sum kdim integers
      use mpimod

      integer ki(kdim)
      integer tmp(kdim)

      call mpi_reduce(ki,tmp,kdim,mpi_itype,MPI_SUM,NROOT,myworld,mpinfo)
      if (mypid == NROOT) ki = tmp
