...
!$TAF STORE dqt,dp,dq  = inner1
      if(nprc == 1) call kuo
!$TAF store dp,dq,dqdt,dtdt = inner1
!$TAF store dprcl,dprscl = inner1
...
!
      ...
      if(ntime ==1) call mksecond(zsec1,0.)
      if(nclouds ==1) then
!$TAF store dprc,dqdt,dtdt,dp,dq,dudt,dvdt,icclev,icctot,dprll,dprsll = inner1
        call mkclouds
      end if
      if(ntime ==1) then
       call mksecond(zsec1,zsec1)
...
!
      ...
      real zqnh(NHOR)        ! q after condensation first iteration
      real ztn(NHOR)         ! t after condensation
      real ztnh(NHOR)        ! t after condensation first iteration
      real zdqdt(NHOR)       ! local q-tendency
      real zdqdth(NHOR)      ! local q-tendency first iteration
...
!
     ...
     &           *zqsat(:)*zcor(:)/(zt(:)-ra4)**2)
        ...
        zqnh(:)=zq(:)+zdqdth(:)
        zqsat(:)=rdbrv*ra1*exp(ra2*(ztnh(:)-TMELT)/(ztnh(:)-ra4))         &
     &          /(dp(:)*sigma(jlev))
!
!      avoid negative qsat
!
...
!
        ...
        zqsnl=rdbrv*ra1*exp(ra2*(ztnew1-TMELT)/(ztnew1-ra4))              &
     &       /(dp(jhor)*sigma(NLEV))
!
!      avoid negative qsat
!
...
!
         ...
         zqsnl=rdbrv*ra1*exp(ra2*(ztnew2-TMELT)/(ztnew2-ra4))           &
     &        /(dp(jhor)*sigma(NLEV))
!
!      avoid negative qsat
!
...
!
         ...
         zqsnl=rdbrv*ra1*exp(ra2*(ztnew4-TMELT)/(ztnew4-ra4))           &
     &        /(dp(jhor)*sigma(jlev))
!
!      avoid negative qsat
!
...
!
          ...
          zqsnl=rdbrv*ra1*exp(ra2*(ztnew5-TMELT)/(ztnew5-ra4))          &
     &         /(dp(jhor)*sigma(jlev))
!
!      avoid negative qsat
!
...
!     check for buyoncy
!
!$TAF incomplete zqp,ztp,itop,ilift,zdqdt,zdtdt,zpt
...
!
           ...
           zqsnl=rdbrv*ra1*exp(ra2*(ztnew8-TMELT)/(ztnew8-ra4))            &
     &          /(dp(jhor)*sigma(jlep))
!
!      avoid negative qsat
!
...
!
     ...
     &             +zdqtot(jhor,jlep)*dsigma(jlep)*zale(jhor,jlep)
       ...
       enddo
      enddo
!$TAF STORE ilift = inner1

!
!*    3) search for top level
!
!$TAF STORE zte = inner1

!$TAF incomplete ztp
      do jlev=NLEM,1,-1
       jlep=jlev+1
       zfac=(sigma(jlev)/sigma(jlep))**akap
!$TAF STORE itop,zqp,ztp = innern
!$TAF incomplete ztp,zpt,zpq,zbeta,zpbeta,zdtdt,zdqdt,itop,zi
!$TAF LOOP = parallel
       do jhor=1,NHOR
!$TAF incomplete ztp,zpt,zpq,zbeta,zpbeta,zdtdt,zdqdt,itop,zi
...
!
         ...
         zqsnl=rdbrv*ra1*exp(ra2*(ztnew10-TMELT)/(ztnew10-ra4))           &
     &        /(dp(jhor)*sigma(jlev))
!
!      avoid negative qsat
!
...
!
          ...
          zqsnl=rdbrv*ra1*exp(ra2*(ztnew11-TMELT)/(ztnew11-ra4))            &
     &         /(dp(jhor)*sigma(jlev))
!
!      avoid negative qsat
!
...
!$TAF incomplete ztp,zpt,zpq,zqp,zbeta,zpbeta,zdtdt,zdqdt,itop,zi
...
!
     ...
     &              +zpt(jhor)
           zpq(jhor)=zdqdt(jhor,jlev)*dsigma(jlev)*zale(jhor,jlev)     &
     &              +zpq(jhor)
!$TAF incomplete zpt,zpq,zbeta,zpbeta
           if(kbeta > 0 ) then
            zbeta(jhor)=AMIN1(1.,zqe(jhor,jlev)/zqsa)*dsigma(jlev)     &
     &                  +zbeta(jhor)
...
!
!$taf STORE zdqdt,zdtdt,zpbetta,zi,zpt,zpq = inner1
...
!
     ...
     &       .and. ishallow(:) == 0)
        ...
        endwhere
       enddo
!$TAF STORE zsums = inner1
       where(zsums(:) > 0.)
        zup(:)=zup(:)/zsums(:)
        zvp(:)=zvp(:)/zsums(:)
       endwhere
!$TAF STORE zup, zvp = inner1
...
!
!$TAF STORE zebs,zqn = mkshallow, REC = 1
...
!     semi implicit scheme
      zebs = 0. ! FO: to make recomputation easier for TAF
      ztn  = 0. ! FO: to make recomputation easier for TAF
!
!     top layer elimination
!
      where(kshallow(:) > 0)
...
!     bottom layer elimination
!
!$TAF store zebs,ztn = mkshallow, REC = 1
      where(kshallow(:) > 0) 
...
!$TAF loop = sequential
       do jlev=NLEM,1,-1
        ...
        endwhere
       enddo
!$TAF store zun,zvn = mkshallow, REC = 1
!
!     tendencies
!
       do jlev=1,NLEV
...
!$TAF loop = sequential
     ...
     &               /(dsigma(NLEV)+zkdiff(:,NLEM)*(1.-zebs(:,NLEM)))
        endwhere
!$TAF STORE zebs(:,1:NLEM),zken(:,2:NLEV) = mkshallow, REC = 1
!$TAF loop = sequential
        do jlev=NLEM,1,-1
         jlep=jlev+1
         where(kshallow(:) > 0)
         ...
         endwhere
        enddo
!$TAF STORE zken(:,:) = mkshallow, REC = 1
!
...
!
 ...
 1000 continue
       ...
       ztht(:)=0.
       zqt(:)=0.
!$taf incomplete icon, ibase, itop
       do jlev=1,NLEM
        jlep=jlev+1
!$TAF store zth(:,jlev:jlev+1),ztht,icon,ibase,itop,zsum1,zsum2,zsumq1,zsumq2,zqt = mkdca
     ...
     &           +zskap(jlev)*dsigma(jlev)*zth(:,jlev)
        ...
        endwhere
       enddo
!$TAF store icon = mkdca
       if(SUM(icon(:)) > 0) then
        do jlev=NLEM-1,1,-1
!$TAF store zth(:,jlev),ztht,icon,ibase,itop,zsum1,zsum2,zsumq1,zsumq2,zqt = mkdca
...
!$TAF store itop, ibase = mkdca
        ...
        enddo
        jiter=jiter+1
! original:        if(jiter > 2*NLEV) goto 1001
        if(jiter > 2*NLEV) error stop "goto 1001 -- jiter > 2*NLEV in mkdca" ! FO: to aid flow normalisation
        goto 1000
      endif
!$TAF store zth,zqn = mkdca
!$TAF incomplete dtdt,dqdt
      do jlev=1,NLEV
...
!
      do jlev=1,NLEV
!$TAF store zprc,zprl,zprsc,zprsl = mkrain
