...
!
      real zkdiffm(NHOR,NLEM),zkdiffh(NHOR,NLEM)
...
!
       ...
       do jlev=2,NLEM
        jlem=jlev-1
        zebs2(:,jlev)=zkdiffm(:,jlev)                                    &
     &              /(dsigma(jlev)+zkdiffm(:,jlev)                      &
     &               +zkdiffm(:,jlem)*(1.-zebs2(:,jlem)))
     ...
     &             /(dsigma(NLEV)+zkdiffm(:,NLEM)*(1.-zebs2(:,NLEM)))
!$TAF INCOMPLETE zken
       do jlev=NLEM,1,-1
        jlep=jlev+1
...
!
      do jlev=2,NLEM
       jlem=jlev-1
       zebs2(:,jlev)=zkdiffh(:,jlev)                                     &
     &             /(dsigma(jlev)+zkdiffh(:,jlev)                       &
     &              +zkdiffh(:,jlem)*(1.-zebs2(:,jlem)))
...
!
     ...
     &           /(dsigma(NLEV)+zkdiffh(:,NLEM)*(1.-zebs2(:,NLEM)))
!
!     back-substitution
!
!$TAF INCOMPLETE zqn
...
!
      do jlev=2,NLEM
       jlem=jlev-1
       zebs3(:,jlev)=zkdiffh2(:,jlev)                                     &
     &             /(dsigma(jlev)+(zkdiffh2(:,jlev)                      &
     &              +zkdiffh2(:,jlem)*(1.-zebs3(:,jlem)/zskap(jlem)))     &
     &              /zskap(jlev))
       ztn(:,jlev)=(zt(:,jlev)*dsigma(jlev)                             &
     &             +zkdiffh2(:,jlem)/zskap(jlem)*ztn(:,jlem))            &
     &            /(dsigma(jlev)+(zkdiffh2(:,jlev)                       &
     &              +zkdiffh2(:,jlem)*(1.-zebs3(:,jlem)/zskap(jlem)))     &
     &              /zskap(jlev))
      enddo
...
!
     ...
     &            +zkdiffh2(:,NLEM)*ztn(:,NLEM)/zskap(NLEM))             &
     &           /(dsigma(NLEV)+zkdiffh2(:,NLEM)/zskap(NLEV)             &
     &                         *(1.-zebs3(:,NLEM)/zskap(NLEM)))
!
!     back-substitution
!
!$TAF INCOMPLETE ztn
