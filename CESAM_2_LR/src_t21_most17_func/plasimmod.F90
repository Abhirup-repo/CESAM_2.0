...
!      *********************
      ...
      integer :: nud = 106 ! plasim_diag 
      integer :: nut = 107 ! reusable temporary unit
...
!     ***************
      ...
      integer :: nstep2          =       0 ! start timestep for this run

!!    for MIT coupling
      integer :: kcplstep        =     -99 ! coupling step, calculated as ocean step / atm step 
	                                       ! if not set explicitely in plasim_namelist
      integer :: nstep0          =       0 ! start timestep, added, FastOpt
      integer :: nstepe          =       0 ! end timestep, added, FastOpt
      integer :: n_inner_loop    =      16 ! FastOpt: length of inner loop (for checkpointing)
      integer :: nstep_inner               ! counter of master loop
      integer :: n_spinup        =       0 ! FastOpt: additional spinup timesteps
      logical :: lunstable       = .false. ! added, FastOpt
      integer :: ncost           =       0 ! Cost function evaluation interval; added, FastOpt
!!   
...
!!    for MIT coupling
      integer :: genobs   = 0   ! switch for writing of observations !SiS
...
! Guokun edits 
     ! define 2d parameter matrix
      ...
      real :: albgmaxg(NUGP)=0.  ! guokun included for 2d par
      real :: albgmaxm(NHOR)=0.
      real :: alpha1g(NUGP)=0.
      real :: beta1g(NUGP)=0.
      real :: gamma1g(NUGP)=0.
      real :: alpha1m(NHOR)=0.
...
! Guokun's modifications       
      real :: rcritg(NUGP,NLEV)=0.  ! guokun included for 2d par
      real :: rcritm(NHOR,NLEV)=0.
...
!!    for MIT coupling
      real :: gaussw(NLAT)    ! Gaussian weights, FastOpt
!!
