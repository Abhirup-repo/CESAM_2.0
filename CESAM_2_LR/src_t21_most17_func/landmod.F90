...
!
         ...
         if(dglac(jhor) > 0.5) then
          ...
          dsnow(jhor)=dsnowz(jhor)
          zdalb=(albgmaxm(jhor)-albgmin)*(dts(jhor)-263.16)/(TMELT-263.16)
          dalb(jhor)=MAX(albgmin,MIN(albgmaxm(jhor),albgmaxm(jhor)-zdalb))
          drhs(jhor)=1.0
         end if
...
!
       ...
       dtsoil(:) = dsoilt(:,1)             ! FO: reset variables which were initialised at program start only
       if(NLSOIL > 2) dtd2(:)=dsoilt(:,2)  ! FO: reset variables which were initialised at program start only
       if(NLSOIL > 3) dtd3(:)=dsoilt(:,3)  ! FO: reset variables which were initialised at program start only
       if(NLSOIL > 4) dtd4(:)=dsoilt(:,4)  ! FO: reset variables which were initialised at program start only
       dtd5(:)   = dsoilt(:,NLSOIL)        ! FO: reset variables which were initialised at program start only
...
!       end where
      do jhor=1,NHOR
         if(dglac(jhor) == 1. .and. dls(jhor) > 0.0)then

       dalb(jhor)=MAX(albgmin,MIN(albgmaxm(jhor)                                  &
     &  ,albgmaxm(jhor)-(albgmaxm(jhor)-albgmin)*(dts(jhor)-263.16)/(TMELT-263.16)))
       drhs(jhor)=1.0
         endif
      enddo
      return
...
!
      do jlev=1,NLSOIL
       ...
       elsewhere
        zcap(:,jlev)=0. ! FO: to avoid incomplete detection
       endwhere
      enddo
!
...
!$TAF init mktsoil = static, 1
!$TAF init mktsoil_n = static, NLSOIL-2
!
...
!
!$TAF incomplete zdiff, zcap, zebs, ztn
      do jlev=NLSOIL-1,2,-1
       jlep=jlev+1
       jlem=jlev-1
!$TAF store zebs(:,jlev+1), ztn(:,jlev+1) = mktsoil_n
       where(dls(:) > 0.0)
        zdiff(:,jlem)=2.*pdiff(:,jlev)*pdiff(:,jlem)                    &
     &                /(pdiff(:,jlev)*psoilz(:,jlem)                    &
     &                 +pdiff(:,jlem)*psoilz(:,jlev))
     ...
     &                           +zdiff(:,jlev)*ztn(:,jlep))
       endwhere
      enddo
!$TAF store zdiff, zcap, zebs, ztn = mktsoil, REC = 1
!
...
!     back-substitution
!
!$TAF store zdiff, zebs, ztn = mktsoil, REC = 1
!$TAF loop = sequential      
      do jlev=2,NLSOIL
       jlem=jlev-1
       where(dls(:) > 0.0)
...
!
        ...
        where ( dglac(:) > 0.5 ) ! FO: for TAF, nested where makes dependencies clearer in this case
