...
!     convert time step to date / time
      ...
      idatim(:) = 0
      monaccu(:) = 0 ! avoid recomputation
      if (kperp > 0) then                     ! perpetual date
        if (n_days_per_year == 365) call step2cal(kstep,ntspd,idatim) ! make sure monaccu is set
...
!     compute weight
      ...
      !     ====================
!     SUBROUTINE MMDD2YDAY
!     ====================

      subroutine mmdd2yday(kyday,kyear,kmon,kday)
      use calmod
      implicit none
      integer,intent(in) :: kyear,kmon,kday
      integer, intent(out) :: kyday
      logical :: leap 
      integer :: iy400,id400
      integer :: iy100,id100
      integer :: iy004,id004
   
      iy400 = kyear   /  400    ! segment [400]
      id400 = mod(kyear ,400)   ! year in segment [0..399]
      iy100 = id400   / 100    ! century [0,1,2,3]
      id100 = mod(id400,100)   ! year in century [0..99]
      iy004 = id100   /   4    ! tetrade [0..24]
      id004 = mod(id100,  4)   ! year in tetrade [0,1,2,3]
      ...
      leap  = (id004 == 0 .and. (id100 /= 0 .or. id400 == 0))
      if(leap) then
         kyday=mona365(kmon-1)+kday
      else
	    if(n_days_per_year == 365 ) then
          kyday=mona365(kmon-1)+kday
        else
          kyday=(kmon-1)*30+kday
        endif
      endif
! guokun added here for 360 days
      if(n_days_per_year == 360 ) then 
         kyday=(kmon-1)*30+kday
      endif
! end guokun's revisement

      if(n_days_per_year == 365 ) then   
        if(kyear ==2101 ) then ! counting for year 1992
          kyday=kyday+365
        endif
        if(kyear ==2102 ) then ! counting for year 1992
          kyday=kyday+365+365
        endif
        if(kyear ==2103 ) then ! counting for year 1992
          kyday=kyday+365+365+365
        endif        
        if(kyear ==2104 ) then ! counting for year 1992
          kyday=kyday+365+365+365+366
        endif   
        if(kyear ==2105 ) then ! counting for year 1992
          kyday=kyday+365+365+365+366+365
        endif   
      else
        ! if(kyear>=2131)then
        if(kyear>=2461)then
           kyday=kyday+360*(kyear-2461)
        endif
      endif 
       
      return
      end subroutine mmdd2yday
