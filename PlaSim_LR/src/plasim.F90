...
!     additional filenames for time control SiS     
      plasim_time_control = trim(plasim_time_control) // mrext
...
!	  write(*,*)'opendiag, NROOT=',NROOT,' mypid=',mypid
      if (mypid == NROOT) then
         open(nud,file=plasim_diag)
         open(nutc,file=plasim_time_control)
      endif
...
! n_run_years is added to n_run_months in subroutine readnl, 
! 360days/365 days is considered in cal2step
call cal2step(nstepe,ntspd,ndatim(1)+(ndatim(2)+n_run_months)/12,mod(ndatim(2)+n_run_months,12), &
              & ndatim(3)+n_run_days,ndatim(4),ndatim(5)) ! added, FastOpt
...
!     =================
      ...
      subroutine master
      use pumamod
! FO: variables required in tapes:
      use oceanmod, only: nstep_ocean => nstep
      use oceanmod, only: yfldo,yifluxr,ysst
      use seamod,  only: dts_sea => dts
      use seamod,  only: dqs_sea => dqs
      use seamod,  only: naccua,cheata,clhdta,clhfla,clwfla,cpmea,croffa,cshdta,cshfla, &
           cswfla,ctauxa,ctauya,cust3a,cheata,cprsa,cicec,ciced,cmld,csmelt, &
           csndch,csnow,csst,cts
      use icemod,  only: nstep_ice => nstep
      use icemod,  only: naccuo,cheat,xicec,xicecc,xiced,xoflux,xqmelt,xsnow,xsst,xts
      use landmod, only: dts_land => dts
      use landmod, only: dqs_land => dqs
      use landmod, only: dsnowz,dsoilt,dtclim,dtsm,dwater,dwclim
      use radmod, only: gdist2
      use rainmod, only: dprcl,dprll,dprscl,dprsll,icclev,icctot
      use radmod, only: dqo3,gmu0
      use observation, only: addcost,genav
! /FO
      use tafmod
      real :: zstart,zstop     ! for cpu time control output S1S
      real :: zt_gridpointa=0  ! for cpu time control output S1S
      real :: zt_spectrala=0   ! for cpu time control output S1S
      real :: zt_gridpointd=0  ! for cpu time control output S1S
      real :: zt_spectrald=0   ! for cpu time control output S1S


      write(*,*)'entering master'
!
!     ***************************
...
!$TAF STORE ndatim = main
      ikits = nkits
      do jkits=1,ikits
!$TAF STORE nkits, nstep, naccua, naccuo, ndatim =  euler, REC = jkits
!$taf STORE dcsoil,dforest,dicec,dnogrow,dql,dqt,dres,drhs,dsnow,dsnowz,dsoilt,&
!$TAF&      dtclim,dtsm,dvlai,dvrhs,dvsoil,  &
!$taf&      dvveg,dvz0,dwatc,dwclim,dwmax,nbiomass,xicec,xiced,ysst,zfw,zveg &
!$taf&      = euler, REC = jkits
!$taf STORE icclev,icctot,ndivdamp,&
!$taf&      cheat,cheata,cprsa,dalb,dcc,dfd,dflux,dlwfl,dprcl,dprll,dprscl,dprsll,dq,dqo3, &
!$taf&      dsnowz,dsoilt,dswfl,dt,dtclim,dts,dts,dtsa,dtsm,&
!$taf&      du,dv,dwater,dz0,gdist2,gmu0,monaccu,naccua,ndatim,nstep,sd,sdm,sdp,sp,spm,&
!$taf&      spp,sq,sqm,sqp,st,stm,stp,sz,szm,szp,xicec,xicecc,xiced,xmld,xoflux,xqmelt,&
!$taf&      xsnow,xsst,xts,yfldo,yifluxr &
!$taf&      =  euler, REC = jkits
...
!        endif
!$TAF STORE nkits,dq,sd,sp,sq,st,sz =  euler, REC = jkits
         call gridpointa
!$TAF STORE sqt =  euler, REC = jkits
         call makebm
!$TAF STORE bm1 =  euler, REC = jkits
         call spectrala
!$TAF STORE sdp,sqp,stp,szp,naccuo,nstep_ocean,xtfreeze =  euler, REC = jkits
!$TAF STORE sd,sp,sq,st,sz,dq,dt,du,dv,dw =  euler, REC = jkits
         call gridpointd
!$taf store dp,dq,dqt,dt,sdt,sqt,stt,szt =  euler, REC = jkits
!$TAF STORE sdm,spm,spp,spt,sqm,stm,stt,szm =  euler, REC = jkits
         call spectrald
!$TAF STORE sdm,spm,spp,spt,sqm,stm,stt,szm =  euler, REC = jkits
         nkits = nkits - 1
      enddo
!$TAF STORE nkits,ndatim,nstep,nstep_ice,nstep_ocean, &
!$TAF&      gdist2,naccua,naccuo,ndivdamp,cheat,cheata,clhdta,clhfla,clwfla, &
!$TAF&      cpmea,cprsa,croffa,cshdta,cshfla,cswfla,ctauxa,ctauya,cust3a,dalb,dcc, &
!$TAF&      dfd,dflux,dfores,dicec,dlhdt,dlwfl,&
!$TAF&      dprcl,dprll,dprscl,dprsll,dql,dqo3,dqt,drhs,drunoff,dshdt,dsnow,dsnowz,&
!$TAF&      dsoilt,dswfl,dtclim,dts_land,dts_sea,dtsa,   &
!$TAF&      dtsm,du,dust3,dv,dwatc,dwater,dwclim,dwmac,dz0,gmu0,icclev,icctot,xicec,&
!$TAF&      xicecc,xiced,xmld,      xoflux,xqmelt,ndatim, &
!$TAF&      xsnow,xsst,xts,yfldo,yifluxr,ysst = main, IF (ikits>=1)
!$TAF STORE dcveg = main, IF (ikits>=1.and.nveg>0)
...
!     ****************************************************************
      ...
      delt2    = delt + delt
      call makebm
!$TAF store bm1, nkits = main
!!for taf      if (mypid == NROOT .and. nsela > 0) then
!!for taf         call tracer_ini
!!for taf      endif
...
!$TAF STORE ndatim = main
      ...
      nstep0=nstep
!$TAF store nstep,nstep0,szp,szm,sz,stp,stm,st,sqp,sqm,sq,spp,spm,sp,sdp,sdm,sd,gz,gv,gu,gt,gq,gpj,gp,gd,dt,dq = main
!$TAF STORE cheata,cprsa,dprcl,dprll,dprscl,dprsll,dqo3,dsnowz,dsoilt,dtclim,dts,dts,dtsm,gdist2,&
!$TAF&      gmu0,monaccu,naccua,nstep,xicec,xiced,xsnow,xsst,xts,xmld = main
...
!$taf STORE &
!$taf&      cheata,cprsa,dalb,dcc,dflux,dlwfl,dprcl,dprll,dprscl,dprsll,dq,dqo3,dsnowz,dsoilt,dswfl,dt,dtclim,dts,dts,dtsa,dtsm,&
!$taf&      du,dv,dz0,gdist2,gmu0,monaccu,naccua,ndatim,nstep,sd,sdm,sdp,sp,spm,spp,sq,sqm,sqp,st,stm,stp,sz,szm,szp,xicec,xiced,&
!$taf&      xsnow,xsst,xts &
!$taf& = main
...
!$TAF STORE sd,sp,sq,st,sz,dq,dt,du,dv,dw = main
!        *****************************
...
!$taf store dp,dq,dqt,dt,sdt,sqt,stt,szt= main
         if (mypid == NROOT) then
...
!$TAF STORE ndatim = main
      ...
      if (mocd == 0 .and. nscd  > 0) then ! placed before month change SiS
         nscd = nscd - 1
      endif
...
!!for taf         if (nshutdown   > 0) return

!! replaced to above SiS   if (mocd == 0 .and. nscd  > 0) then
!! replaced to above SiS     nscd = nscd - 1
!!for taf            if (nscd == 0) return
!! replaced to above SiS   endif

    enddo ! month countdown
!close(420)
!time control output SiS
write(nutc,*)'number of timesteps             :',nstep
write(nutc,*)'used time acc [s] in gridpointa : ',zt_gridpointa
write(nutc,*)'used time acc [s] in spectrala  : ',zt_spectrala
write(nutc,*)'used time acc [s] in gridpointd : ',zt_gridpointd
write(nutc,*)'used time acc [s] in spectrald  : ',zt_spectrala
...
!
       ...
       enddo
       du = du ! workaround for bug in tlm generation up to and including TAF-5.7.7
       dv = dv ! workaround for bug in tlm generation up to and including TAF-5.7.7
!
...
!     =====================
      ...
      subroutine gridpointd
        use pumamod
        use seamod, only: dqsh => dqs
        use seamod, only: dtsh => dts
...
!
      ...
      call fluxstep
!$TAF store devap,dlhfl,dshfl,dtaux,dtauy,dtsa,dudt,dvdt,dtdt,dqdt = main     
!
!     dbug print out
!
...
!$TAF store gdist2,dalb,dcc,dqo3,gmu0,dfd,dflux,dlwfl,dswfl,dtdt = main     
!
!     dbug print out
!
...
!
!$TAF store dp,dprc,dprl,dprs,dq,dqdt = main
      call surfstep
