...
!$TAF SUBROUTINE get_restart_integer  INPUT = 1,2
...
!$TAF SUBROUTINE get_restart_array  INPUT = 1,2,3,4,5
...
!
      ...
      character (len=*) :: yn
      real :: pa(k2,k3)
      real :: pa4(k2,k3) ! FO: for compilation with -r8
      
      do j = 1 , nresnum
         if (trim(yn) == trim(yresnam(j))) then
            call reseek(yn,j)
            read (nreaunit) pa4(1:k1,:)
            pa(1:k1,:) = pa4(1:k1,:)
            nlastrec = nlastrec + 1
            return
         endif
...
!$TAF SUBROUTINE get_restart_seed  INPUT = 1,2,3
