...
!$TAF STORE dqt,dp,dq  = main
      if(nprc == 1) call kuo
!$TAF store dp,dq,dqdt,dtdt = main
!$TAF store dprcl,dprscl = main
...
!
      ...
      if(ntime ==1) call mksecond(zsec1,0.)
      if(nclouds ==1) then
!$TAF store dprc,dqdt,dtdt,dp,dq,dudt,dvdt,icclev,icctot,dprll,dprsll = main
        call mkclouds
      end if
      if(ntime ==1) then
       call mksecond(zsec1,zsec1)
...
!$TAF incomplete ztp
      do jlev=NLEM,1,-1
       jlep=jlev+1
       zfac=(sigma(jlev)/sigma(jlep))**akap
!$TAF STORE itop,zqp,ztp = main_nlev
...
!
!$taf STORE zdqdt,zdtdt,zpbetta,zi,zpt,zpq = main
...
!
       ideep(:)=0
!$taf store ilift,itop = main_nlev
...
!
     ...
     &       .and. ishallow(:) == 0)
        ...
        endwhere
       enddo
!$TAF STORE zsums = main
       where(zsums(:) > 0.)
        zup(:)=zup(:)/zsums(:)
        zvp(:)=zvp(:)/zsums(:)
       endwhere
!$TAF STORE zup, zvp = main
...
!
      kktop(:)=1
      dp = dp ! to fix dottest, taf
      dq = dq ! to fix dottest, taf
